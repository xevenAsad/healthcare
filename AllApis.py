...
def save_user():
    ...
    try:
        ...
        cursor.execute("SELECT Email FROM UserDetails WHERE Email = ?", (email,))
        existing_user = cursor.fetchone()
        if existing_user:
            return json.dumps({'status': 'error', 'message': 'User with this email already exists!'})

        # Insert the user details into the database
        db.insert_user_details(email, username, phone, company, total_blogs, remaining_blogs, active_status, profile_pic, description,Password)
...
def get_users():
    try:
        cursor = db.conn.cursor()
        cursor.execute("SELECT Email, UserName, Phone, Company, TotalBlogs, RemainingBlogs, ActiveStatus FROM UserDetails")
